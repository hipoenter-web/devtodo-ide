...
| 과제 항목 | 구현 상태 | 구현 내용 |
| --- | --- | --- |
| Web IDE 기본 UI 설계 | 완료 | 프로젝트 트리, Activity Log, Live Preview, Code Editor, Project Review 패널로 화면을 구성했습니다. View 메뉴로 각 패널을 열고 닫을 수 있으며, Preview와 로그 패널은 직접 높이를 조절할 수 있습니다. |
| 직관적인 레이아웃과 반응형 화면 | 완료 | 데스크톱에서는 IDE형 3열 레이아웃을 사용하고, 좁은 화면에서는 세로 흐름으로 사용할 수 있게 구성했습니다. Preview는 PC/Mobile 보기 전환을 제공합니다. |
| 로그인 기능 | 부분 구현 | 회원가입과 비밀번호 재설정은 제외하고, 과제 시연용 로그인과 역할별 권한을 구현했습니다. Master, Team, Client 권한에 따라 프로젝트 접근, Todo 관리, 댓글 작성, 코드 저장 가능 범위를 분리했습니다. |
| 코드 편집기 기능 | 부분 구현 | 실제 폴더를 열어 파일 트리를 탐색하고, 텍스트 파일을 코드 편집기에서 확인·수정할 수 있습니다. 브라우저 파일 권한을 지원하는 환경에서는 Save로 원본 파일 저장이 가능하며, 권한이 없으면 세션 저장으로 대체합니다. 새 파일/폴더 생성 기능은 향후 확장 범위로 정리했습니다. |
| 채팅 기능 | 부분 구현 | 실시간 WebSocket 채팅 대신 프로젝트별 리뷰 코멘트 기능을 구현했습니다. 코멘트는 Express API를 통해 MongoDB에 저장되며, 메시지/작성자/역할 기준 검색과 MongoDB 텍스트 인덱스 구조를 추가했습니다. |
| Git 저장소 생성 및 히스토리 관리 | 완료 | GitHub 저장소와 main 브랜치를 사용하고, 기능 단위로 commit/push하여 Render 배포와 연결했습니다. |
| 프로젝트 배포 | 완료 | Render Web Service에 배포했으며, Express 서버가 React 빌드 결과와 API를 함께 제공합니다. MongoDB Atlas 연결 시 Todo와 리뷰 코멘트가 서버 DB에 저장됩니다. |
| 제출 자료 정리 | 진행 | README에 실행 방법, 폴더 구조, 데이터 흐름, 권한 설계, 배포 방법, 향후 개선 방향을 정리했습니다. |
...
배포된 Render 서버와 MongoDB Atlas를 함께 사용하므로 여러 사용자가 같은 배포 주소에 동시에 접속하고, 같은 프로젝트의 Todo와 리뷰 코멘트를 공유할 수 있습니다. 현재 채팅은 안정성을 우선한 서버 저장형 코멘트 방식입니다.

- 구현: 프로젝트별 코멘트 작성, MongoDB 저장, 키워드 검색, 작성자/역할 표시
- 가능: 여러 사용자가 같은 프로젝트 데이터를 조회하고 코멘트를 남기는 동시 사용
- 미구현: WebSocket 기반 실시간 메시지 push, 타이핑 표시, 온라인 접속자 목록
- 향후 확장: `presence` API 또는 Socket.io/WebSocket을 추가해 최근 활동자와 실시간 접속자를 표시할 수 있습니다.

## Render 배포

이 프로젝트는 과제의 “프로젝트 배포” 요구를 충족하기 위해 Render 단일 Web Service 배포를 기준으로 구성했습니다. AWS 대신 Render를 사용한 이유는 Node.js 서버와 React 정적 빌드를 하나의 Web Service에서 빠르게 배포하고, GitHub push 기반 자동 배포 흐름을 확인하기 위해서입니다. `render.yaml`에는 다음 흐름이 정의되어 있습니다.
...
- WebSocket 기반 실시간 채팅과 접속자 presence 표시
